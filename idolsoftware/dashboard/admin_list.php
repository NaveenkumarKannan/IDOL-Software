...
if($ses_user_id!='1')
{
	 ...
	 $sql="select * from attendance where $get_query102 att_id!='' and assign_user_id='$ses_user_id' order by att_id asc";
}
else
{
	 $sql="select * from attendance where $get_query102 att_id!='' order by att_id asc";
					...
					?>
                                        ...
                                        <td><?php echo $i; ?></td>
											...
											<td><?php echo date("H:i" ,strtotime($record['start_time'])); ?><img src="assets/img/loc.png" onClick="get_start_map('<?php echo $sal_per_name['person_name']; ?>','<?php echo $record['lattitude']; ?>','<?php echo $record['longitude']; ?>','<?php echo $record['start_time']; ?>')" height="25" width="30"/></td>
											<td><?php echo date("H:i" ,strtotime($record['end_time'])); ?><img src="assets/img/loc.png" onClick="get_end_map('<?php echo $sal_per_name['person_name']; ?>','<?php echo $record['end_lat']; ?>','<?php echo $record['end_long']; ?>','<?php echo $record['end_time']; ?>')" height="25" width="30"/></td>
