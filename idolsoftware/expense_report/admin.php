...
?>
				<div class="row filter-row">
								...
								<select class="form-control" id="emp_id"> 
									<option value="">Select Employee</option>
									 <?php 
											...
											{
											$user_id=mysql_fetch_array(mysql_query("select * from user_creation where insert_id='$row[sal_per_id]' source='Manage Users'"));
											?>
						...
						<div class="col-sm-3 col-xs-6">  
                        <div class="form-group ">
							<button class="btn btn-primary" style="margin-top: 20px;" type="button" onClick="get_expense_report(emp_id.value,from_date.value,to_date.value)">View Report</button>
                            </div>
						</div>     
                    </div>
					<div class="row">
						<div class="col-md-12">
							 <div id="expense_report_div"> <?php include('admin_list.php') ?></div>
						</div>
					</div>
                </div>
 ...
 <script>
function get_expense_report(emp_id,from_date,to_date)
{//alert("emp_id="+emp_id+"&from_date="+from_date+"&to_date="+to_date);
jQuery.ajax({
type: "POST",
url: "expense_report/admin_list.php",
data: "emp_id="+emp_id+"&from_date="+from_date+"&to_date="+to_date,
success: function(data) {
jQuery("#expense_report_div").html(data);
}
});
}
