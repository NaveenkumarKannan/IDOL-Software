...
if($picturename1!='') {  $new_img=$picturename1."-".$_POST['randomno']."-".$_POST['randomsec']; } else {$new_img=$_POST['img']; }
		...
		$insql12=mysql_query("insert into user_creation(user_name,password,confirm_paassword,user_type,status,insert_id,source,assign_user_id,assign_user_type)values
			('$_POST[username]','$_POST[passwrd]','$_POST[confirmpassword]','3','$_POST[status]','$insert_id','Manage Users','$_POST[ses_user_id]','$_POST[ses_user_type]')");

$upload_file='../assets/img/users/'.$picturename1."-".$_POST['randomno']."-".$_POST['randomsec'];
	...
	$image_array=$_FILES['file_data'];
		$picturename1=$image_array['name'];
		$main_name=$image_array['tmp_name'];
		if($picturename1!='') { $new_img=$picturename1."-".$_POST['random_no']."-".$_POST['random_sec']; } else {$new_img=$_POST['img']; }
		 $sql = "select * from sale_person_creation where  person_name='$_POST[personname]' and
